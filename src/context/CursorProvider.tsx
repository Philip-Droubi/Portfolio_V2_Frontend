...
const GREENS = ["#00ff88", "#22c55e", "#4ade80", "#86efac"];
...
export default function CursorProvider({ children }: { children: React.ReactNode }) {
    const zones = useRef<Set<HTMLElement>>(new Set());
    const [particles, setParticles] = useState<Particle[]>([]);

    const registerZone = (el: HTMLElement | null) => {
        if (el) zones.current.add(el);
    };

    const unregisterZone = (el: HTMLElement | null) => {
        if (el) zones.current.delete(el);
    };

    const isInZone = (target: HTMLElement | null) => {
        if (!target) return false;

        for (const zone of zones.current) {
            if (zone.contains(target)) return true;
        }
    ...
    useEffect(() => {
        ...
        const handleMove = (e: MouseEvent) => {
            ...
            // 🚫 only allow navbar/footer zones
            if (!isInZone(target)) return;
    ...
    return (
        <CursorContext.Provider value={{ registerZone, unregisterZone, isInZone }}>
            {children}
                ...
                {particles.map((particle) => (
                    <span
                        ...
                        style={{
                            ...
                            ["--rotate" as any]: `${particle.rotation}deg`,
                        }}
                    >
                        {particle.value}
                    </span>
